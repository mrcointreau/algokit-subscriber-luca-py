...
def test_fails_if_too_far_from_tip() -> None:
    ...
    last_txn_round = send_x_transactions(2, algorand.account.localnet_dispenser().address, algorand)["last_txn_round"]
    ...
    assert fail_message == f"Invalid round number to subscribe from 1; current round number is {last_txn_round}"


def test_does_not_fail_if_not_too_far_from_tip() -> None:
    algorand: AlgorandClient = AlgorandClient.default_localnet()
    result = send_x_transactions(2, algorand.account.localnet_dispenser().address, algorand)
