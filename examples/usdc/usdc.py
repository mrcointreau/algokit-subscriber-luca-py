...
def print_usdc(transaction: SubscribedTransaction, filter_name: str) -> None:
    ...
    * The filter name (from the 'filters' list) that the transaction matched
    ...
    Here we are only using this EventListener callback for one filter, but if we had multiple filters
    we could use the filter name to determine which filter the transaction matched.
    """
